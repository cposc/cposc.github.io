    ...
    <div class="scheduleDetails">
      ...
      <div v-if="sessions && sessions.length > 0" class="sessionBlocks">
        <div v-for="(session, index) in sessions" v-bind:key="index" class="sessionBlock">
          ...
          <h1 v-if="index === 0" class="aud">The Auditorium / Steinman Hall - <a :href="'/2024/speakers#' + session.speaker[0].split(' ').join('+')" target="_blank">{{ session.speaker[0] }}</a><span v-if="session.speaker[1]"> <span>{{ session.speaker[2] ? "," : "&" }}</span> {{ session.speaker[1] }}</span><span v-if="session.speaker[2]">, & {{ session.speaker[2] }}</span></h1>
          <h1 v-if="index === 1" class="binns">
            Binns Room - <a :href="'/2024/speakers#' + session.speaker[0].split(' ').join('+')" target="_blank">
              {{ session.speaker[0] }}
            </a>
          ...
          <h1 v-if="index === 2" class="library">Library - <a :href="'/2024/speakers#' + session?.speaker[0].split(' ').join('+')" target="_blank">{{ session?.speaker[0] }}</a><span v-if="session.speaker[1]"> & <a :href="'/2024/speakers#' + session.speaker[1].split(' ').join('+')" target="_blank">{{ session.speaker[1] }}</a></span></h1>
